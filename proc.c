...
int ps() {
    ...
    acquire(&ptable.lock);
    cprintf("name \t pid \t state \t priority \t nice  \t tickets \n");
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->state == SLEEPING)
            cprintf("%s \t %d \t SLEEPING \t %d \t %d \t %d\n ", p->name, p->pid, p->priority, p->nice, p->tickets);
...
int ticket(int pid, int tickets){
    struct proc *p;
